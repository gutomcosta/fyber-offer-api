...
describe Offer do
  ...
  describe ".build" do 
    ...
    it "buids an Offer object from an Hash" do 
      offer = Offer.build(data).first
      expect(offer.title).to eq("Tap  Fish")
      expect(offer.thumbnail).to eq("http://cdn.sponsorpay.com/assets/1808/icon175x175-2_square_60.png")
      expect(offer.payout).to eq(90)
    end
